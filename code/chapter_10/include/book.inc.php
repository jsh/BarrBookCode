...
// Buckets
define('BOOK_BUCKET', 'atetlaw-sitepoint-book');
...
// Status values in BOOK_FEED_DOMAIN
 ...
 * findQueueURL -
 *
 *  Locate the queue URL for the given queue. Return
 *  the URL on success, null on error.
 */
...
function findQueueURL($sqs, $queueName)
{
  ...
  if ($res->isOK())
  {
    $queueURL = (string) $res->body->CreateQueueResult->QueueUrl;
    return $queueURL;
  }
  else
  {
 ...
 *  available.  Return the queue URL, the raw message,
...
function pullMessage($sqs, $queueURL)
{
  while (true)
  {
    ...
    {
      print("Could not pull message from queue '${queueURL}': " .
 ...
 * getItemAttributes - 
 *	
 *	Intended for use with an item returned by SimpleDB, return
 *	an associative array of attribute values. The array is
 *	indexed by attribute name and contains scalar values for
 *	single-valued attributes and arrays of scalar values for
